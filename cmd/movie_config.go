// movie_config.go — movie movie config
package cmd
...
Examples:
  movie movie config                           # Show all
  movie movie config get movies_dir            # Get one
  movie movie config set movies_dir ~/Movies   # Set one
  movie movie config set tmdb_api_key abc123   # Set API key`,
	Run: runMovieConfig,
}
